...
function SurveyList(){const [search,setSearch]=useState("");const [status,setStatus]=useState("");const [from,setFrom]=useState("");const [to,setTo]=useState("");const [page,setPage]=useState(1);const url=`/surveys?page=${page}&pageSize=15&search=${encodeURIComponent(search)}&status=${status}&from=${from}&to=${to}`;const state=useLoad(()=>api<PageData>(url),[url]);return <><div className="admin-toolbar"><div className="admin-search"><Search size={17}/><input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} placeholder="Название или протокол"/></div><select value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="">Все статусы</option>{Object.entries(statusNames).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><input aria-label="Период от" type="date" value={from} onChange={e=>{setFrom(e.target.value);setPage(1)}}/><input aria-label="Период до" type="date" value={to} onChange={e=>{setTo(e.target.value);setPage(1)}}/><a className="admin-button primary" href="/admin/surveys/new"><Plus size={17}/>Новый опрос</a></div>{state.loading?<Loading/>:state.error?<ErrorBox message={state.error}/>:!state.data?.items.length?<Empty title="Опросы не найдены"/>:<Panel title={`Опросы · ${state.data.total}`}><DataTable headers={["Опрос","Протокол","Статус","Версия","Период","Вопросы","Eligible","Completed",""]}>{state.data.items.map(raw=>{const row=raw as Survey;return <tr key={row.id}><td><strong>{row.titleRu}</strong><small>{row.titleKk||"KZ не задан"}</small></td><td>№{row.protocolNumber}</td><td><Badge status={row.status}/></td><td>v{row.version}</td><td>{formatDate(row.startsAt)}<small>до {formatDate(row.closesAt)}</small></td><td>{row.questionCount}</td><td>{row.eligibleCount}</td><td>{row.completedCount}</td><td><a className="admin-icon-link" href={`/admin/surveys/${row.id}`} aria-label="Открыть"><ChevronRight size={18}/></a></td></tr>})}</DataTable><Pagination current={page} total={state.data.total} size={state.data.pageSize} onChange={setPage}/></Panel>}</>}
